...
/*==========================================================================
	Someday i will learn how to fly - phreaker
==========================================================================*/
    if (isset($data->collectcrop)) {
        if ($data->collectcrop == "1" && $StopHarvest == 0)
        {
            $reload = 0;
            $bot->SendMsg('Harvesting Crops');
            if(count($data->collectCropsData) ==0) $bot->SendMsg('You did not select any crops in Crops Plugins.');
            foreach ($bot->fobjects as $obj)
            {
                if (($obj["className"] == "Plot") && ($obj["state"] != "withered") && ($obj["state"] != "plowed") && in_array($obj["contractName"], $data->collectCropsData))
                {
               ...
               foreach ($bot->order_visitor_help as $Nuid => $val)
                {
                  ...
                  if(is_array($val['helpTargets']) && $val['status'] == "unclaimed" && isset($val['senderID']) && count($val['helpTargets']) > 0)
                  {                                                   //status  String
